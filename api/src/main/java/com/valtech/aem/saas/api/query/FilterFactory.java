...
public final class FilterFactory {
   ...
   * @return a composite filter with an 'OR' joining operator
   */
  public static Filter createFilter(@NonNull String key, FilterJoinOperator joinOperator,
      @NonNull List<String> values) {
    if (CollectionUtils.isNotEmpty(values)) {
      if (values.size() > 1) {
        return CompositeFilter.builder()
            .joinOperator(joinOperator)
            .filters(values.stream().map(v -> createFilter(key, v)).collect(Collectors.toSet()))
            .build();
      }
