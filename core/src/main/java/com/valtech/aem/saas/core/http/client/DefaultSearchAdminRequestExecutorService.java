...
@Slf4j
@Component(name = "Search as a Service - Search Admin Request Executor Service", service = SearchAdminRequestExecutorService.class, immediate = true, configurationPid = CONFIGURATION_PID)
...
public class DefaultSearchAdminRequestExecutorService implements SearchAdminRequestExecutorService {
    ...
    @ObjectClassDefinition(name = "Search as a Service - Search Admin Request Executor Service Configuration", description = "URL and authentication details for connect to Search Admin Endpoint.")
    public @interface Configuration {
        ...
        @AttributeDefinition(name = "Base URL", description = "The protocol + url for the search service") String searchAdminRequestExecutorService_baseurl() default DEFAULT_WEB_SERVICE_URL; // NOSONAR

        @AttributeDefinition(name = "Basic authentication - User", description = "User for basic authentication to the search service") String searchAdminRequestExecutorService_basicAuthentication_user(); // NOSONAR

        @AttributeDefinition(name = "Basic authentication - Password", description = "Password for basic authentication to the search service", type = AttributeType.PASSWORD) String searchAdminRequestExecutorService_basicAuthentication_password(); // NOSONAR

        @AttributeDefinition(name = "Use basic authentication", description = "Submit the above user + password to the search service", type = AttributeType.BOOLEAN) boolean searchAdminRequestExecutorService_basicAuthentication_enable() default DEFAULT_BASIC_AUTHENTICATION_ENABLE; // NOSONAR

        @AttributeDefinition(name = "JWT authentication token", description = "Token string", type = AttributeType.PASSWORD) String searchAdminRequestExecutorService_jwtAuthentication_token(); // NOSONAR

        @AttributeDefinition(name = "Use JWT authentication", description = "Set above authorization token in search service request.", type = AttributeType.BOOLEAN) boolean searchAdminRequestExecutorService_jwtAuthentication_enable() default DEFAULT_JWT_AUTHENTICATION_ENABLE; // NOSONAR
